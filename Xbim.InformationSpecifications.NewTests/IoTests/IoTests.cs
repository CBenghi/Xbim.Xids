...
public partial class IoTests
{
	...
	public void CanSaveAndOpenMultipleRegexPatterns()
	{
		...
		var occurrences = patternRegex().Matches(file); // the file has it
	...
	[Fact]
	public void ShouldNotLoadInvalidXml()
	{
	...
	public void CanRoundTripFacetRequirementCardinality(IFacet facet, RequirementCardinalityOptions.Cardinality cardinality)
	{
		...
		var newFacet = newRequirement!.Facets.FirstOrDefault();
		var newCardinality = newRequirement.GetRequirementCardinalityOption(newFacet!, out var v);
		newCardinality.Should().BeTrue("Expected to find cardinality option for facet");
		if (newCardinality)
		{
			v.Should().Be(cardinality);
		}
	}
	...
	private static readonly RequirementCardinalityOptions.Cardinality[] cardinalities = [
		RequirementCardinalityOptions.Cardinality.Optional,
		RequirementCardinalityOptions.Cardinality.Prohibited,
		RequirementCardinalityOptions.Cardinality.Expected
		];
	...
	private Xids BuildMultiSpecGroupIDS()
	{
		var file = new FileInfo(@"bsFiles/bsFilesSelf/TestFile.ids");
