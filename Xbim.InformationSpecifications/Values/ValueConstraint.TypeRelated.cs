...
namespace Xbim.InformationSpecifications
{
	public partial class ValueConstraint
	{
		...
		/// <summary>
		/// Converts a Xids value to its IDS string representation based on the specified type name.
		/// </summary>
		public static string? PersistIdsValue(object? value, NetTypeName typeName)
		{
			if (value is null)
				return null;
			try
		...
		private static string AsXsDurationString(TimeSpan ts)
		{
			// XSD duration format: P[nY][nM][nD][T[nH][nM][nS]]
			// Since TimeSpan doesn't track years and months, we convert everything to days, hours, minutes, seconds
		...
		internal static bool XidsIsValid(string? minValue, NetTypeName baseType, [NotNullWhen(true)] out string? asIds)
		{
			if (minValue is null)
			{
				asIds = null;
			...
			var tmp = ParseValue(minValue, baseType);
			if (tmp is null)
			{
				asIds = string.Empty;
				return false;
			}
			asIds = PersistIdsValue(tmp, baseType);
			return asIds != null;
		}
	}
